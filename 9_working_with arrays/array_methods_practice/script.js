...
//1. getting all the movements in a single array & filtering only the deposit and adding all together
...
//2. count how many deposits in the bank with atleast 1000
const numDeposits1000 = accounts
  .flatMap(acc => acc.movements)
  .filter(mov => mov >= 1000).length;
console.log(numDeposits1000);

// doing the above action using the reduce method

const usingRedu = accounts
  .flatMap(acc => acc.movements)
  .reduce((count, cur) => (cur >= 1000 ? ++count : count), 0);
// .reduce((count, cur) => (cur >= 1000 ? count + 1 : count), 0);
console.log(usingRedu);
// we cant use count++ as the ++ operator increase the value and it returns the previous value. but we can use the prefix ++ operator i.e. ++count

// 3. using the reduce method in advance to creat a object instead of just numbers or strings. Tp create an object which contains the sum of the deposits and of the withdrawals

const { deposits, withdrawals } = accounts
  .flatMap(acc => acc.movements)
  .reduce(
    (sums, cur) => {
      // cur > 0 ? (sums.deposits += cur) : (sums.withdrawals += cur); // we can do this part in a different way by fixing duplication
...
// 4. To create a simple function to convert any string to title case(converting first letter of every word to uppercase)
// this is a nice title -> This Is a Nice Title

const convertTitleCase = function (title) {
  const capitalize = str => str[0].toUpperCase() + str.slice(1);
  const exceptions = ['a', 'b', 'and', 'the', 'but', 'or', 'on', 'in', 'with'];

  const titleCase = title
    .toLowerCase()
    .split(' ')
    .map(
      word => (exceptions.includes(word) ? word : capitalize(word)) // word[0].toUpperCase() + word.slice(1)
    )
    .join(' ');
