...
console.log(nitheesh['lastName']); // bracket notation the
...
/*const nitheesh = {
    ...
    calcAge: function () {
        this.age = 2024 - this.birthYear;// we can store the value in a property of the object, age is the property of the object
        return this.age; // this keyword is used to refer to the object itself the object the 'this' keyword is referring to is object nitheesh
    },
...
console.log(nitheesh.calcAge());
console.log(nitheesh.age); // the value stored in the property age is printed rather than loging the calcAge function repeatedly
console.log(nitheesh.age);
console.log(nitheesh.age);
console.log(nitheesh.getSummary());

// Challenge
//"Nitheesh is a 27-year old student, and he has a/no driver's license" log this in console */
...
// Instead of writing the same code 10 times we can use a for loop to do the same thing
...
/*const nitheesh = [
    ...
    2037 - 2000,
    'student',
    ['Shrey', 'Dileep', 'Shubham'],
    true
];
const types = []; //creating a new array using the types of the nitheesh array and logging it to the console

for (let i = 0; i < nitheesh.length; i++) {
    console.log(nitheesh[i], typeof nitheesh[i]);

    // Filling types array
    // types[i] = typeof nitheesh[i]; // this is one way of doing it
    types.push(typeof nitheesh[i]); // this is another way of doing it
}
console.log(types);

// Another example pushing the ages to the array ages by calculating the ages from the birth years in the array years
...
for (let i = 0; i < nitheesh.length; i++) {
    if (typeof nitheesh[i] !== 'string') continue; // continue statement will exit the current iteration of the loop and continue to the next iteration
    console.log(nitheesh[i], typeof nitheesh[i]);
}
...
for (let i = 0; i < nitheesh.length; i++) {
    if (typeof nitheesh[i] === 'number') break; // break statement will exit the loop completely
    console.log(nitheesh[i], typeof nitheesh[i]);
} */
...
for (let i = nitheesh.length - 1; i >= 0; i--) { //nitheesh.length - 1 is the last index of
    console.log(i, nitheesh[i]); // i is used to get the index of the array
}

for (let exercise = 1; exercise < 4; exercise++) {
    console.log(`-------- Starting exercise ${exercise}`);

    for (let rep = 1; rep < 6; rep++) { // this is a loop inside a loop
        console.log(`Exercise ${exercise}: Lifting weights repetition ${rep} 🏋️‍♂️`); // the ${exercise} is used to get the esercise number from the exercise for loop
    }
} */
...
let dice = Math.trunc(Math.random() * 6) + 1; // this is a random number generator, the turnc method is used to remove the decimal part of the number and the random method is used to generate a random number.
console.log(dice);

while (dice !== 6) {
    console.log(`You rolled a ${dice}`);
    dice = Math.trunc(Math.random() * 6) + 1; //using this again to generate a new random number after the first number is generated
    if (dice === 6) console.log('Loop is about to end...'); // this will be printed when the dice is 6
} */
